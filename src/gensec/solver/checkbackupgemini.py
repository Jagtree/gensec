...
### TODO: verify the implementation below.

### """Implementation Notes (Evolution from initial solution):
###     -------------------------------------------------------
###     1. Origin-Preserving Scaling:
###        Scaling is performed using the maximum absolute value per axis (L-inf norm).
###        This ensures the origin (0,0,0) remains fixed at the relative center of 
###        the scaled space. This is critical for radial utilization (eta_3D) 
###        consistency, as standard min-max scaling can shift the origin 
###        asymmetrically when the section is rotated.
### 
###     2. Robust Qhull Configuration (QJ & Qt):
###        - 'QJ' (Joggled Input): Adds a tiny internal perturbation to handle 
###          perfectly co-planar points. This prevents topological errors when 
###          processing domains generated through planar scans.
###        - 'Qt' (Triangulated Output): Forces the hull into a triangular mesh, 
###          ensuring more stable ray-intersection results across rotated geometries.
### 
###     3. Refined Ray-Cast Engine:
###        Intersection thresholds (denom > 1e-13) are optimized to distinguish 
###        between grazing (tangent) rays and valid boundary hits, minimizing 
###        discrepancies between 0° and 90° orientations.
### """
...
class DomainChecker:
    ...
    def __init__(self, nm_3d):
        ...
        # Numerical Scaling:
        # Scaling by the maximum absolute value per axis ensures that the
        # origin (0,0,0) remains at the center of the coordinate system,
        # which is vital for radial utilization (eta_3D) consistency.
        self._scale = np.max(np.abs(pts), axis=0)
        self._scale[self._scale < 1e-8] = 1.0
        
        pts_scaled = pts / self._scale

        # 'QJ' (Joggled input) prevents precision errors with co-planar points.
        # 'Qt' (Triangulated output) ensures the hull is made of triangles, 
        # which provides more stable ray-intersection results.
        self.hull = ConvexHull(pts_scaled, qhull_options='QJ Qt')
    ...
    def _scale_point(self, p):
        """Map physical coordinates to scaled space."""
    ...
    def is_inside(self, N, Mx, My=0.0):
        ...
        bool
        """
        p_raw = self._make_point(N, Mx, My)
        p = self._scale_point(p_raw)
        
        # Checking against all half-spaces: A*p + c <= 0
        return bool(np.all(
            self._equations[:, :-1] @ p + self._equations[:, -1] <= 1e-9
        ))
    ...
    def _ray_eta(self, base_raw, target_raw):
        r"""
        ...
        Parameters
        ----------
        base_raw : numpy.ndarray
        target_raw : numpy.ndarray
        ...
        float
            :math:`\eta`.
        """
        base = self._scale_point(base_raw)
        target = self._scale_point(target_raw)
        
        direction = target - base
        dist = np.linalg.norm(direction)
        
        if dist < 1e-12:
            return 0.0 if self.is_inside(*base_raw) else float("inf")
        ...
        # Filter for intersections in front of the ray.
        # We use a slightly more generous threshold for denom to avoid
        # grazing intersections that could lead to large t values.
        valid = denom > 1e-13
        if not np.any(valid):
            return 0.0 if self.is_inside(*base_raw) else float("inf")
            
        t_values = numer[valid] / denom[valid]
        t_positive = t_values[t_values > 1e-12]
        
        if len(t_positive) == 0:
            return 0.0 if self.is_inside(*base_raw) else float("inf")
            
        # The first intersection is the boundary.
        t_bnd = float(np.min(t_positive))
        return 1.0 / t_bnd
